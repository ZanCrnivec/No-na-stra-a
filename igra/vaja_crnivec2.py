#   Nabirat mors palce in jih dajat v ogenj da se čas ne izteče
...
class Stick:
    def __init__(self, x, y, image):
        self.x = x
        self.y = y
        self.image = pygame.transform.scale(image, (120, 70))
    def draw(self, display, scroll):
        display.blit(self.image, (self.x - scroll[0], self.y - scroll[1]))
...
class Tema:
    def __init__(self, x, y, width, height, color, alpha):
        ...
        self.alpha = alpha
    def draw(self, display):
        pygame.draw.rect(display, self.color, (self.x, self.y, self.width, self.height), self.alpha)

tema = Tema(0, 0, 1000, 800, (0,0,0), pygame.SRCALPHA)
...
while True:
    ...
    if check_proximity(500, 400, campfire.x - display_scroll[0], campfire.y - display_scroll[1]):
        timer += stick_inv*2
        stick_inv = 0
    ...
    tema.alpha = int(blizu_ognja(500, 400, campfire.x - display_scroll[0], campfire.y - display_scroll[1])* 255 / 1000)
    
    print(blizu_ognja(500, 400, campfire.x - display_scroll[0], campfire.y - display_scroll[1]))

    draw_text("Palce: ", text_font, (255,255,255), 10, 1, stick_inv)
    draw_text("Čas: ", text_font, (255,255,255), 10, 40, int(timer//1))
